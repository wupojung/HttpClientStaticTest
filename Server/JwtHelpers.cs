...
namespace Server
{
    public class JwtHelpers
    {
        ...
        public static string GetClaim(string accessToken, string claimKey)
        {
            string result = null;
            try
            {
                //if (!accessToken.ValidateJwtToken())
                //{
                //    throw new Exception("accessToken was not JWT token");
                //}
                //解析內容
                var token = new JwtSecurityToken(accessToken);
                result = token.Claims.Where(c => c.Type == claimKey).Select(x => x.Value).First();
            }
            catch (Exception exp)
            {
